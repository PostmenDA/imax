...
_ga.trackSocial=function(a,c,b){_ga.trackFacebook(a,c);_ga.trackTwitter(a,c);_ga.trackVkontakte(a,c,b)};
_ga.trackFacebook=function(a,c){var b=_ga.buildTrackerName_(c);try{FB&&FB.Event&&FB.Event.subscribe&&(FB.Event.subscribe("edge.create",function(d){_gaq.push([b+"_trackSocial","facebook","like",d,a])}),FB.Event.subscribe("edge.remove",function(d){_gaq.push([b+"_trackSocial","facebook","unlike",d,a])}),FB.Event.subscribe("message.send",function(d){_gaq.push([b+"_trackSocial","facebook","send",d,a])}))}catch(e){}};_ga.buildTrackerName_=function(a){return a?a+".":""};
_ga.trackTwitter=function(a,c){var b=_ga.buildTrackerName_(c);try{twttr&&twttr.events&&twttr.events.bind&&twttr.events.bind("tweet",function(d){if(d){var c;d.target&&"IFRAME"==d.target.nodeName&&(c=_ga.extractParamFromUri_(d.target.src,"url"));_gaq.push([b+"_trackSocial","twitter","tweet",c,a])}})}catch(e){}};
_ga.trackVkontakte=function(a,c,b){var e=_ga.buildTrackerName_(c);try{VK&&VK.Observer&&VK.Observer.subscribe&&(VK.Observer.subscribe("widgets.like.liked",function(){_gaq.push([e+"_trackSocial","vkontakte","like",b,a])}),VK.Observer.subscribe("widgets.like.unliked",function(){_gaq.push([e+"_trackSocial","vkontakte","unlike",b,a])}))}catch(d){}};
_ga.extractParamFromUri_=function(a,c){if(a){var a=a.split("#")[0],b=a.split("?");if(1!=b.length)for(var b=decodeURI(b[1]),c=c+"=",b=b.split("&"),e=0,d;d=b[e];++e)if(0===d.indexOf(c))return unescape(d.split("=")[1])}};
/* * * * */
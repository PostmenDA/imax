...
var MovieReminder = $.inherit({
	__constructor: function(oOptions){
		...
		this.sCity = $("#currentCitySpan").attr("abbr");
		this.init();
	},
	init: function(){
		var oThis = this;
...
$(function(){
	...
	$.cookie('lang', window.sLang, {expires: 30, path: '/',domain: '.planetakino.ua'});
	...
	$.eventBus.bind("user-not-authorized", function(event, data){
		$("body").removeClass("authorized-user");
                $("body").removeClass("pre-authorized-user");
...
//		$("body").addClass("authorized-user");
  ...
  $('body').on('click', function(){
  	$('.addon-menu__container').addClass('nodisplay');
  });
...
$(document).ready(function () {
    ...
    if ($.cookie('pk_city') && $.cookie('pk_city') != '') {
        if (currentCity == 'kiev' && window.location.pathname.slice(1, 5).toString() != 'kiev') {
            // есть кука с городом и переход не по ссылке с указанием кинотеатра.
...
$(document).ready(function() {
    ...
    if($("#quick-info").hasClass("alt-links-1")) {
        var p = $( "#quick-info" );
        var offset = p.offset();    
        var left = offset.left;    
        var top = offset.top;
        var loginwidth = $(".navigation-section #auth_block").width(); 
        
        if(doc_w < 1462){
            if(left < 1100){
                if(top < 310){
                    $( ".right-panel .navigation-section .addon-menu__container").addClass("black_bg"); 
                }
            };
            if(left < 560){            
                $( ".right-panel .navigation-section .addon-menu__container").removeClass("black_bg");           
            };        
            if(left < 1050){
                if(top < 510){
                    $( ".left-panel .navigation-section .addon-menu__container").addClass("black_bg"); 
                }
            } 
            if(left < 424){            
                $( ".left-panel .navigation-section .addon-menu__container").removeClass("black_bg");            
            }; 
        }; /* width < 1462px /*/   
        if(doc_w < 1200){
           if(left > 430){
        ...
        if(loginwidth < 255){$( ".right-panel .navigation-section .addon-menu__container").addClass("black_bg");}  
    }           
});
$(window).resize(function() {
    ...
    if($("#quick-info").hasClass("alt-links-1")) {
        ...
        var top = offset.top;
        var loginwidth = $(".navigation-section #auth_block").width();
        $( ".navigation-section .addon-menu__container").removeClass("black_bg"); 
        if(doc_w < 1462){
            if(left < 1100){
                if(top < 310){
                    $( ".right-panel .navigation-section .addon-menu__container").addClass("black_bg"); 
...
$(document).ready(function() {
    $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").addClass("nodisplay");
    
    $( ".left-panel .navigation-section .main-addon-menu .addon-menu__icon-wrapper").click(function (){ 
        if($( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").hasClass("nodisplay")){
            $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").removeClass("nodisplay");
            $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").removeClass("addon-menu_state-closed");
            $( ".left-panel .navigation-section .main-addon-menu").removeClass("addon-menu__closed"); 
            $( ".right-panel .navigation-section .city-selector .addon-menu__container").addClass("nodisplay");
        }
        else{
           $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").addClass("nodisplay");           
            $( ".right-panel .navigation-section .city-selector .addon-menu__container").addClass("nodisplay");
...
$(document).ready(function() {
    ...
    $( ".right-panel .navigation-section .city-selector .addon-menu__icon-wrapper").click(function (){   
        
        if($( ".right-panel .navigation-section .city-selector .addon-menu__container").hasClass("nodisplay")){
            
            $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").addClass("nodisplay");
            $( ".right-panel .navigation-section .city-selector .addon-menu__container").removeClass("nodisplay");
            $( ".right-panel .navigation-section .city-selector .addon-menu__container").removeClass("addon-menu_state-closed");
        }
        else{
            $( ".left-panel .navigation-section .main-addon-menu .addon-menu__container").addClass("nodisplay");
            $( ".right-panel .navigation-section .city-selector .addon-menu__container").addClass("nodisplay");
...
/***** input [type = file] *****/
	    ...
	    $(".file_upload button").text("Обрати");
	    $(".file_upload div").text("Файл не обрано");
...
$(function(){
    $("#blogout").on("click", function(){
     $.ajax({
	  url: 'http://facecab.planetakino.ua/signout',
	  crossDomain: true,
...
$(document).ready(function() {
$(".city-selector .addon-menu__container .addon-menu__list-item a").on("click", function(e){
   var newCity =  $(this).attr("abbr");
   $.cookie('pk_city', newCity, {expires: 30, path: '/',domain: '.planetakino.ua'});
});

$(function(){
	...
	var jComingSoonMoviesList = $(".coming-soon", jMoviesList);
	var jNowPlayingMoviesList = $(".now-playing", jMoviesList);
	...
	function updateColumns(){
		...
		if (jComingSoon.length && jNowPlaying.length) {
			...
			oComingSoon.calculatedWidth = jMoviesList.width() * oComingSoon.count / (oComingSoon.count + oNowPlaying.count) - oComingSoon.padding;
			oNowPlaying.calculatedWidth = jMoviesList.width() * oNowPlaying.count / (oComingSoon.count + oNowPlaying.count) - oNowPlaying.padding;
			
			oComingSoon.cols = oComingSoon.calculatedWidth / oComingSoon.itemWidth;
			oNowPlaying.cols = oNowPlaying.calculatedWidth / oNowPlaying.itemWidth;
			
			oComingSoon.cols > oComingSoon.count && (oComingSoon.cols = oComingSoon.count);
			
			oNowPlaying.cols < 1 && (oNowPlaying.cols = 1);
			oComingSoon.cols < 1 && (oComingSoon.cols = 1);
